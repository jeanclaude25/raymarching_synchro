precision lowp float;
    ...
    #define pi 3.141593
        ...
        uniform vec3 uObjectRotation; //Object rotation
...
{   
    float angle = 2.*pi/uResolution.x;
    float angle2 = -2.*pi/uResolution.y;
...
{
	vec3 rc = texture2D(tDiffuse, ray.xy).rrr;
    for (int l=0; l<3; l++)
        rc+= 1.5*normalize(lamps[l].color)*lamps[l].intensity*specint*pow(max(0.0, dot(ray, normalize(lamps[l].position - cameraPosition))), 200.);
    return rc;
}
...
vec4 render(vec2 fragCoord, vec3 cameraPos, float ior)
{   
  ...
  vec3 ray = GetCameraRayDir(uv, uCamLookAt, fov);
    
  RenderData traceinf = trace0(cameraPos, ray, false);
...
vec4 render_rgb(vec2 fragCoord, vec3 cameraPos)
{
    #ifdef color_disp
    	vec4 col;
    	col.r = render(fragCoord, cameraPos, ior_r).r;
    	col.g = render(fragCoord, cameraPos, ior_g).g;
    	col.b = render(fragCoord, cameraPos, ior_b).b;
    	col.a = 1.;
	return col;
    #else
    	return render(fragCoord, cameraPos, ior);
    #endif
}

vec4 render_aa(vec2 fragCoord, vec3 cameraPos)
{
    // Antialiasing
    vec4 vs = vec4(0.);
    for (int j=0;j<aasamples ;j++)
    {
       ...
       for (int i=0;i<aasamples ;i++)
       {
          ...
          vs+= render_rgb(fragCoord + vec2(ox, oy), cameraPos);
       }
    }
    return vs/vec4(aasamples*aasamples);
		...
		void main() {
            ...
            vec2 uv = ((vUv - .5) * uResolution) / uResolution.y;
            col += render_aa(uv, cameraPosition).xyz;
